...
// 图像数据回调，将image_msg放入image_buf，同时根据时间戳检测是否是新的图像序列
...
// imu前向递推的回调函数，从imu预积分的位姿得到imu位姿和camera位姿，得到低延迟和高频率结果
...
// 重定位回调函数，将重定位帧的相对位姿放入 loop_info，updateKeyFrameLoop() 进行回环更新
...
// VIO回调函数，根据pose_msg中的位姿得到imu位姿和camera位姿
...
void process()
{
    ...
    while (true)// 
    {
        ...
        if (pose_msg != NULL)
        {
            ...
            // 将距上一关键帧距离 （平移向量的模）超过 SKIP_DIS 的图像创建为关键帧
            if((T - last_t).norm() > SKIP_DIS)
            {
                ...
                m_process.lock();
                start_flag = 1;
                // 在 posegraph 中添加关键帧，flag_detect_loop=1 回环检测
...
int main(int argc, char **argv)
{
    ...
    if (LOOP_CLOSURE)
    {
        ...
        posegraph.loadVocabulary(vocabulary_file); // 读取字典
    ...
    measurement_process = std::thread(process); // posegraph 主线程
    keyboard_command_process = std::thread(command); // 键盘操作的主线程
